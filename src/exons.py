...
def saveGenomicRecords(records, filepath):
    ...
    pickle.dump(genomic_records, file_path)

def openGenomicRecords(filepath):

    file = open(filepath, 'rb')
    genomic_records = pickle.load(file)
    return genomic_records
...
def mapExonBoundariesToAlignment(records, genomic_records):
    ...
    for record in records:
        ...
        skip_records = ['XP_010997363.1', 'XP_006162319.2', 'XP_005406339.2', 'XP_014050304.1']
...
# filepath = 'original_2U1.obj'
# filepath = 'problem.obj'
filepath = 'leo.obj'

saveGenomicRecords(leo, filepath)
...
mapExonBoundariesToAlignment(leo, genomic_records)
